# Programme pour calculer les moyennes et determiner le classement des etudiants
"""
...
for i in range(nombre_etudiants):
    print(f"\nSaisie des notes pour l'etudiant {i+1}:")

    # Recuperation des 3 notes
    somme_notes = 0
    for j in range(3):
        while True:
            try:
                ...
                if 0 <= note <= 20:
                    ...
                    print("Erreur: La note doit etre comprise entre 0 et 20.")
            except ValueError:
...
for i in range(nombre_etudiants):
    print(f"Etudiant {numeros_etudiants[i]}: Moyenne = {moyennes[i]:.2f}/20")
...
print("\n=== CLASSEMENT ===")
print(f"Premier de la classe: Etudiant {numeros_etudiants[index_premier]} avec une moyenne de {meilleure_moyenne:.2f}/20")
print(f"Dernier de la classe: Etudiant {numeros_etudiants[index_dernier]} avec une moyenne de {pire_moyenne:.2f}/20")

"""dans une version plus ameliorer on aurait pu aller jusqu'a
